...
public:
	...
	virtual Point point(int i) const { return points[i]; }
...
private:
	...
	int r = 0;
public:
	Box(Point lt, int w, int h, int ar) : width(w), height(h), r(ar) //ar 크기에 대한 오류 처리는 하지 않았다.
	{	
		add(Point{ lt.x, lt.y }); // 조건에 따라 Left-top position 저장

		add(Point{ point(0).x, point(0).y}); //끝부분을 원형으로 만들기 위해 각 point를 Shape 클래스의 points 벡터에 추가적으로 저장함.
		add(Point{ point(0).x + getWidth() - r - r, point(0).y + r - r });
		add(Point{ point(0).x + r - r, point(0).y + getHeight() - r - r });
		add(Point{ point(0).x + getWidth() - r - r, point(0).y + getHeight() - r - r });
	}

	int getWidth() const { return width; } //getter
	int getHeight() const { return height; }
	int getArc() const { return r; }
...
private:
	...
	int r = 0;
public:
	Regular_polygon(Point center, int s, int r) : sides(s), r(r)
	{
		Shape::add(Point{ center.x, center.y });
		Shape::add(Point{ point(0).x, point(0).y - getRadius() });
		nextPoint();
	}
...
private:
	...
	int outside_r = 0;
public:
	Star(Point center, int v, int inr, int outr) : vertices(v), inside_r(inr), outside_r(outr)
	{
		Shape::add(Point{ center.x, center.y });
		Shape::add(Point{ point(0).x, point(0).y - getOutsideR() });
		nextPoint();
	}

	int numberOfVertices() const { return vertices; }
	int getOutsideR() const { return outside_r; }
	int getInsideR() const { return inside_r; }
